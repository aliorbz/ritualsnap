...
const Gallery: React.FC<GalleryProps> = ({ frames, onUseFrame }) => {
  ...
  return (
    /* Added pt-12 md:pt-16 to provide space for the sticky header in the gallery view */
    <div className="space-y-8 md:space-y-12 py-4 md:py-8 pt-12 md:pt-16">
      <div className="text-center space-y-2 md:space-y-4">
        <h2 className="text-2xl md:text-4xl font-mystical neon-text tracking-widest uppercase px-4">The Vault of Elements</h2>
        <p className="text-gray-400 max-w-xl mx-auto uppercase text-[8px] md:text-xs tracking-[0.3em] px-4">Ancient tools preserved for eternity</p>
      </div>
      ...
      {frames.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-20 mx-4 border border-dashed border-green-900/30 rounded-3xl bg-black/40 animate-pulse">
          ...
          <p className="text-[10px] text-zinc-700 uppercase tracking-widest mt-2 max-w-xs text-center leading-relaxed px-4">
            No ancient sigils have been bound to this cycle yet. The architect must forge new relics.
          </p>
        </div>
      ) : (
              ...
              <div className="aspect-[3/4] overflow-hidden relative bg-[radial-gradient(circle,_#111_0%,_#000_100%)] flex items-center justify-center">
                ...
                <div className="absolute bottom-2 left-2 right-2 md:bottom-4 md:left-4 md:right-4 z-30">
                  ...
                  <p className={`text-[7px] md:text-[10px] tracking-widest uppercase ${loadErrors[frame.id] ? 'text-red-900' : 'text-green-500'}`}>
                    {loadErrors[frame.id] ? 'Path Broken' : 'Sigil Unlocked'}
                  </p>
                </div>
              </div>
            </div>
      ...
      {selectedDetails && (
        /* Changed fixed inset-0 to top-16/20 so the navbar remains visible when viewing lore */
              ...
              <div className="p-6 md:p-8 space-y-6 flex flex-col">
                ...
                {selectedDetails.creator && (
                  <div className="pt-6 border-t border-zinc-900 space-y-4">
                    <h4 className="text-[10px] font-bold text-green-500 uppercase tracking-[0.2em]">The Architect</h4>
